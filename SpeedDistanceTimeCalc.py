...
if int(Opselect) == 1:
    ...
    print("The distance you travelled is: %.1f miles" % (distanceCalc(float(speed),ffmpegMins(time))))
elif int(Opselect) == 2:
    distance = input("Input miles to travel: ")
    time = input("Input timeframe in mins or hrs e.g. 60m or 2h: ")
    print("The speed you must travel to reach your destination in time is: %.1f mph" % speedCalc(float(distance),ffmpegMins(time)))
elif int(Opselect) == 3:
